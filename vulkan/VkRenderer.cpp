...
void VkRenderer::handleMousePositionEvents(double xPos, double yPos){
  ...
  if (mMouseLock) {
    mRenderData.rdViewAzimuth += (float)mouseMoveRelX / 10.0f;
    ...
    mRenderData.rdViewElevation -= (float)mouseMoveRelY / 10.0f;
...
bool VkRenderer::draw() {
  ...
  mRenderData.rdTickDiff = (float)(tickTime - mLastTickTime);
  ...
  *mModelMesh = mModel->getVertexData();
  mRenderData.rdTriangleCount = (unsigned int)(mModelMesh->vertices.size() / 3);
  ...
  mLineIndexCount =
      (unsigned int)mStartPosArrowMesh.vertices.size() + (unsigned int)mEndPosArrowMesh.vertices.size() +
      (unsigned int)mQuatPosArrowMesh.vertices.size() + (unsigned int)mCoordArrowsMesh.vertices.size() +
      (unsigned int)mSplineMesh.vertices.size();
