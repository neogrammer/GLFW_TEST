...
void VkRenderer::handleMousePositionEvents(double xPos, double yPos) {
    ...
    if (mMouseLock) {
        mRenderData.rdViewAzimuth += mouseMoveRelX / 10.f;
        ...
        mRenderData.rdViewElevation -= mouseMoveRelY / 10.f;
...
bool VkRenderer::draw() {
    ...
    *mEulerModelMesh = mModel->getVertexData();
    mRenderData.rdTriangleCount = (unsigned)(mEulerModelMesh->vertices.size() / 3);
    ...
    *mQuatModelMesh = mModel->getVertexData();
    mRenderData.rdTriangleCount += (unsigned)(mQuatModelMesh->vertices.size() / 3);
    ...
    mLineIndexCount = (unsigned)(mCoordArrowsMesh.vertices.size() + mEulerCoordArrowsMesh.vertices.size() + mQuatArrowMesh.vertices.size());
